...
class SweepCollection:
    ...
    def read_sweep_realtime(self, range, width_bin):
        start = range[0]
        stop = range[1]
        process = Popen(['hackrf_sweep', f'-f {start}:{stop}',f'-w {width_bin}'], stdout=PIPE)
        records = []
        line_count = 0
        sweep = 0
        while process.poll() is None:
            ...
            try:
                if len(line) < 6:
                    raise ValueError(f"items in read line is small: \n {line}")
            ...
            if freq_low == self.freq_min:
                ...
                # print(self.records_on_sweep)
            # # часть строк в коллекции разверток теряется 
            # # оставшуюся часть записей считаем за еще одну развертку
            # self.records_on_sweep.append(line_count)
            if sweep > MAX_RECORDS:
                process.kill()
...
if __name__ == '__main__':
    sc = SweepCollection(range=(70, 110), width_bin=250000)
    print(sc)
    # ts, f_min, f_max, f_step, spectr = sc.get_power_spectr(0)
